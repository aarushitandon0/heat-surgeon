...
Landsat's descending overpass is around **10:30 local time**. This is mid-morning surface temperature, not the mid-afternoon peak. State this in `docs/methodology.md` and in the pitch. Do not let a judge assume we are modelling worst-case heat.
...
```
LST_pred(x) = T_base − k1·canopy(x) − k2·Δalbedo(x) + k3·impervious(x)
```
...
2. Fit `k1, k2, k3, T_base` on that window.
...
|---|---|---|
| Measurement | 30m delivered (100m native) | Real Landsat observation |
| Calibration | 2 km window, hundreds of pixels | Where the coefficients come from |
...
Interventions placed at 2m aggregate into changed land-cover fractions within each 30m model cell. Stated that way it is a coherent resolution story. Hidden, it is the question that sinks the demo.

The UI labels the rendered surface as modelled, not measured. The `/result` payload carries `measurement_resolution_m`, `design_resolution_m`, and `output_kind` so the frontend cannot forget.
...
> "Our model predicts held-out pixels in this neighbourhood to within ±0.71 °C, against a 2.14 °C baseline."
...
| **2** | Land cover + neighbourhood calibration | `k1,k2,k3` fitted; hold-out RMSE printed; coefficient signs sanity-checked |
...
| "How do you know it's accurate?" | Hold-out validation on this neighbourhood's own pixels: ±0.71 °C, against a 2.14 °C mean-prediction baseline. |
...
| "Is this peak heat?" | No. Landsat's overpass is about 10:30 local, so this is mid-morning surface temperature. Afternoon peak would be hotter. |
