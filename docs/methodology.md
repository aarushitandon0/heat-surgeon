...
- **Result (batch of 2026-09-15, `backend/fixtures/ranking/four-windows.json`).**
  - **Coverage.** 204 named streets in the four windows have a way long enough for the 200 m design segment. 56 have a design grid that runs past the window's calibration cells and are not attempted. Of the 148 attempted, 55 have no plantable tree pits and 93 are ranked: 36 in the Bajirao Road window, 24 each in Karve Road and FC Road, 9 in North Main Road.
  - **Ends of the list, by point estimate.** First: Bharatratna Atal Bihari Vajpayee Flyover, −0.97 ± 0.11 °C for 20 trees, 0.82–1.30 °C per ₹1 lakh, ranks 1–27. Last: South Main Road, −0.04 ± 0.01 °C for the 2 trees its strips hold, 0.31–0.48 °C per ₹1 lakh, ranks 87–93.
  - **Ties.** Within the coefficient error, no street's rank is certain. The median street could hold any of 70 ranks; 4,184 of 4,278 pairs of streets have overlapping rank ranges. 52 streets could rank in the top 10, and none is certainly there. The standard error of a street's modelled change is 10–24% of the change (median 14%).
  - **The top is one window.** The first ten by point estimate are all in the Karve Road window, whose fitted canopy coefficient is the highest (7.56, against 5.29–5.68). That coefficient does not differ significantly from the others (transfer test), which is exactly why the top ranks carry ranges of 27 to 76 places.
  - **Fixture streets.** Karve Road ranks 4–42, FC Road 4–81, North Main Road 1–88 (the widest, from that window's larger coefficient errors), Bajirao Road 12–84.
  - **The short search does not always beat the baselines.** On 8 of 93 streets the searched layout cools slightly less than the design guideline, by at most 0.008 °C. Two of those are at full capacity, where every pit is planted and the arms converge. The rest are near capacity, where 150 generations did not find the last small gain. The ranking uses the searched layout's value as found.
  - **What it can support.** Not an ordering of neighbouring streets. It can separate the clearly more cost-effective from the clearly less: for example, streets that could rank only in the 80s and 90s against streets that could not rank below the 40s. That is what the interface shows.
...
## Trees to consider: a cited reference lookup (freeze)

Stage 03 and the printed brief show street tree species as a reference list, set apart from the modelled results. **It is a lookup, not a model output.** Species never enter the fitness function and change no temperature, cost or rank: the model is species-agnostic, and every tree is an 8 m crown (`TREE_CROWN_DIAMETER_M`). The 8 m crown assumption is what ties the list to the model.

**What is shown, and from where** (`frontend/src/lib/species.ts`):
- **The list.** IRC:SP:21-2009 Annex E, trees suitable for the Deccan Plateau, which includes Maharashtra. Botanical names are given in standard spelling where the scanned source misprints them. "Palms", a group rather than a species, is left out.
- **One exclusion.** Polyalthia longifolia is on that national list, but Pune's USDG 2016 §5.2 names Mast Tree (False Ashoka) among trees to avoid. The Pune source wins, and the reason is shown.
- **Size.** IRC:SP:21-2009 Appendix I's own words, quoted, where the scan is legible: Albizia procera, Colvillea racemosa and Mangifera indica large sized; Michelia champaca and Saraca indica small sized. The rest are marked "size not legibly stated".
- **Crown check.** No source gives a mature crown diameter, so no species can be confirmed against 8 m. A small-sized species is flagged as likely to cool less than modelled. A large-sized one is marked consistent but unconfirmed. The rest cannot be checked. The interface says this beside each species.
- **Location.** The street's narrowest tree pit band, from the cross-section, is read against the USDG 2016 §5.2 tree grate table (trunk up to 0.15 m needs a 0.6 m grate; 0.3 m, 0.75 m; 0.9 m, 1.5 m; 1.2 m, 2 m). That gives the largest trunk the street's pits can take. USDG's sentences on tree form by street type are quoted, not used to filter, because they name no widths.
- **Existing trees.** The search already never places a pit on existing canopy, and a new crown over existing canopy counts as zero cooling. The section says so.

**How the sources were found.** Timeboxed to 90 minutes, cited or not shipped:
...
**The limit.** The check that would make the list strongest, each species' crown diameter against the model's 8 m, needs a sourced crown diameter per species. None of these sources gives one, and supplying diameters from memory would put an uncited number on screen. So the list states size in the source's words and never claims a match.
...
- **The public site is this replay, built on its own.** `npm run build:replay` sets `VITE_REPLAY_ONLY`, so the page never tries a backend. Its notice says, on every stage, that the site serves stored results recorded from the satellite and map data committed with the code, and that nothing is computed while the viewer watches. The live backend is deliberately not deployed: it would serve the same cached data, with a running process, cold starts and one more thing to fail in front of a judge.
- **Credits on screen.** Stage 00 lists every data source with its licence (Landsat, USGS public domain; Copernicus Sentinel-2; OpenStreetMap, ODbL; Overture, conflating Microsoft ML Building Footprints and Google Open Buildings) and the cited constants (Ko et al. 2022, Liang 2001, PMC USDG 2016, IRC:SP:21 and IRC:103, RUIDP 2023), in every mode.
